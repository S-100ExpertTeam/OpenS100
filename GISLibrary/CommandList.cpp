...
std::list<S100_Instruction*> CommandList::Parse()
{
	...
	for (auto i = commands.begin(); i != commands.end(); ++i) {
		auto command = i->get();
		auto param = command->getParameter();
		command->parse(param);

		if (dynamic_cast<Part9a::DrawingCommand*>(command)) {
			// PointInstruction
			...
			s.SetStateCommand(dynamic_cast<Part9a::StateCommand*>(command));
		}
	}
...
void CommandList::StateCommandStoreToInstruction(StateCommandStore* stateStore, S100_Instruction* instruction)
{
	if (nullptr == instruction || nullptr == stateStore) {
		return;
	}

	instruction->setId(stateStore->id.has_value() ? stateStore->id->getId() : "");
	instruction->setParentId(stateStore->parent.has_value() ? stateStore->parent->GetParentId() : "");
	instruction->setHover(stateStore->hover.has_value() ? stateStore->hover->GetHover() : false);
	instruction->SetViewingGroup(stateStore->viewingGroup.has_value() ? stateStore->viewingGroup->GetViewingGroups() : std::vector<std::string>());
	instruction->SetDisplayPlane(stateStore->displayPlane.has_value() ? stateStore->displayPlane->GetDisplayPlane() : "");
	instruction->SetDrawingPriority(stateStore->drawingPriority.has_value() ? stateStore->drawingPriority->GetDrawingPriority() : 0);
