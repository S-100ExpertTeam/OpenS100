...
bool PCOutputSchemaManager::ReadPCOutputXML(std::wstring path, S101Cell* cell)
{
	MSXML2::IXMLDOMDocument2Ptr pDoc;							// XML Document Declaration
	...
	if (!path.empty()) 
	{
		pDoc.CreateInstance(__uuidof(MSXML2::DOMDocument));  // create instance

		VARIANT_BOOL ret = pDoc->load(path.c_str());
		auto erorrCode = pDoc->parseError;
		CString msg;
		...
		for (int i = 0; i < pChild->Getlength(); i++)
		{
			...
			if (nodeName.compare(L"displayList") == 0)
			{
				if (displayList)
				{
					delete displayList;
	...
	// Conversion : S100_DisplayList -> SENC_DisplayList
	GenerateSENCInstruction(cell, cell->GetS100Layer()->GetPC());
	GenerateSENCAlertInstruction(cell, cell->GetS100Layer()->GetPC());
	
	InitDisplayList();

	ChangePallete(cell->GetS100Layer()->GetPC());

	return true;
}

bool PCOutputSchemaManager::ReadPCOutputXMLByPugiXML(std::wstring path, S101Cell* cell)
{
	if (!path.empty()) 
	{
		pugi::xml_document doc;
		auto result = doc.load_file(pugi::as_utf8(path).c_str());
		auto displayListNode = doc.child("displayList");

		displayList = new S100_DisplayList();
		for each (auto child in displayListNode)
		{
			displayList->GetContents(child);
		}
	}
	else
	{
