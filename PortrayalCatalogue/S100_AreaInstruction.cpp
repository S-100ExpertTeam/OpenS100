...
void S100_AreaInstruction::GetContents(MSXML2::IXMLDOMNodePtr pNode)
{
	if (!pNode)
		return;
	...
	for (int i = 0; i < pNodeList->Getlength(); i++)
	{
		...
		else if (nodeName.compare(L"symbolFill") == 0)
		{
			if (!areaFill) areaFill = new S100_SymbolFill();
			((S100_SymbolFill*)areaFill)->GetContents(pNode);
		}
		else if (nodeName.compare(L"colorFill") == 0)
		{
			if (!areaFill) areaFill = new S100_ColorFill();
			((S100_ColorFill*)areaFill)->GetContents(pNode);
		}
		else if (nodeName.compare(L"areaFillReference") == 0)
		{
			if (!areaFill) areaFill = new S100_AreaFillReference();
			((S100_AreaFillReference*)areaFill)->GetContents(pNode);
		}
		else if (nodeName.compare(L"pixmapFill") == 0)
		{
			if (!areaFill) areaFill = new S100_PixmapFill();
			((S100_PixmapFill*)areaFill)->GetContents(pNode);
		}
		else if (nodeName.compare(L"hatchFill") == 0)
		{
			if (!areaFill) areaFill = new S100_HatchFill();
			((S100_HatchFill*)areaFill)->GetContents(pNode);
		}
	}
}
