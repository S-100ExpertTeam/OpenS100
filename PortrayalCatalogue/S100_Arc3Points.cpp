...
void S100_Arc3Points::GetContents(MSXML2::IXMLDOMNodePtr pNode)
{
	if (!pNode)
		return;
	...
	for (int i = 0; i < pNodeList->Getlength(); i++)
	{
		...
		else if (nodeName.compare(L"medianPoint") == 0)
		{
			medianPoint.GetContents(pNode);
		}
		else if (nodeName.compare(L"endPoint") == 0)
		{
			endPoint.GetContents(pNode);
		}
	}
}
