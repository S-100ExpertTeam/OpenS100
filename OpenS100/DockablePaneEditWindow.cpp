...
void CDockablePaneEditWindow::AddAttribute(AttributeBinding* ab, CMFCPropertyGridProperty *pSuperProperty, unsigned newParentIndex)
{
	...
	SimpleAttribute* sa = fc->GetSimpleAttribute(attr->Getvalue());
	if (sa) //Define simple and complex.
	{
		AddSimpleAttribute(sa, pSuperProperty, newParentIndex);
	}
	ComplexAttribute* ca = fc->GetComplexAttribute(attr->Getvalue());
	if (ca)
	{
...
void CDockablePaneEditWindow::AddSimpleAttribute(SimpleAttribute* sa, CMFCPropertyGridProperty *pSuperProperty, unsigned parentIndex)
{
	if (m_cell->m_FileType == FILE_S_100_VECTOR)
	{
		...
		int atix = 1;
		CMFCPropertyGridProperty *pProperty = new CMFCPropertyGridProperty(sa->GetName().c_str(), L"");
		pProperty->SetDescription(sa->GetDefinition().c_str());

		if (sa->GetValueType() == FCD::S100_CD_AttributeValueType::enumeration)
		{
			SettingEnumType(sa->GetListedValuesPointer(), pProperty);
		}
		...
		if (m_pFeature != nullptr)
		{
			...
			auto itor = cell->m_dsgir.m_atcs->m_arrFindForCode.find(sa->GetCodeAsWString());
			if (itor == cell->m_dsgir.m_atcs->m_arrFindForCode.end())
			{
				attr->m_natc = GetATCS(sa->GetCodeAsWString());
			}
			else
			{
...
void CDockablePaneEditWindow::AddComplexAttribute(ComplexAttribute* ca, CMFCPropertyGridProperty *pSuperProperty, unsigned parentIndex)
{
	if (m_cell->m_FileType == FILE_S_100_VECTOR)
	{
		...
		// Creating a Property for Complex Attribute
		CMFCPropertyGridProperty *pProperty = new CMFCPropertyGridProperty(ca->GetName().c_str());
		pProperty->SetDescription(ca->GetDefinition().c_str());
		...
		if (m_pFeature != nullptr)
		{
			...
			auto itor = cell->m_dsgir.m_atcs->m_arrFindForCode.find(ca->GetCodeAsWString());
			if (itor == cell->m_dsgir.m_atcs->m_arrFindForCode.end())
			{
				attr->m_natc = GetATCS(ca->GetCodeAsWString());
			}
			else
			{
				attr->m_natc = itor->second->m_nmcd;
		...
		for (auto itor = ca->GetSubAttributeBindingPointer().begin(); itor != ca->GetSubAttributeBindingPointer().end(); itor++)
		{
			auto ab = &(*itor);
			if (ab->GetMultiplicity().GetLower() == 1)
			{
				// Add the essential Sub Attribute of Complex Attribute.
				// I'll hand over Complex Attribute Index.
...
bool CDockablePaneEditWindow::CheckingAttriData(AttributeBinding* ab, CString inputName, CMFCPropertyGridProperty *pSuperProperty)
{
	...
	// The value counts only the value of the parent level.
	//If the number is the maximum, do not allow adding.
	// The test should be done, with the added value now.

	//If there's no added value,
	...
	if (pSuperProperty == nullptr) // In the absence of prior parents,
	{
		...
		attribute = ab->GetMultiplicity().GetUpper().Getattributes();
		for (auto itor = attribute.begin(); itor != attribute.end(); itor++)
		{
			upperAttr = &(*itor);
		...
		if (upperAttr)
		{
			...
			{
				upperCount = _wtoi(ab->GetMultiplicity().GetUpper().Getvalue().c_str());
			}
		}
		...
		for (int i = 0; i < count; i++)
		{
			...
			std::wstring name = pProperty->GetName();
	...
	int upperCount = 0;
	bool isUpper = false; //true, it's infinite.

	auto attri = ab->GetMultiplicity().GetUpper().Getattributes();
	for (auto i = attri.begin(); i != attri.end(); i++)
	{
		...
		if (infi.compare(L"infinite") == 0)
		{
			...
			{
				upperCount = std::stoi(ab->GetMultiplicity().GetUpper().Getvalue());
			}
		}
	}
