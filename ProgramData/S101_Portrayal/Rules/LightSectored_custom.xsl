...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template match="LightSectored[@primitive='Point']" priority="1">
		...
		<xsl:for-each select="sectorCharacteristic/lightSector/sectorLimit">
				...
				<xsl:variable name="length">
					<xsl:choose>
						...
						<xsl:when test="$FULL_SECTORS = 'true' " > <!--  If no valnmr default to 9 -->
				...
				<!--  An augmented ray for each sector leg with Local CRS -->
   				<augmentedRay>
					...
					<crs><xsl:value-of select="$crs"/></crs>  <!-- LocalCRS,  geographicCRS when using actual VALNMR-->
					...
					<direction> <!--  need to flip the bearing by 180 because values are seaward bearing-->
						<xsl:choose>
					...
					</direction> <!--  direction from sector limit +/- 180 -->
					<length><xsl:value-of select="$length"/></length> <!--  Length fixed or nominal range -->
				</augmentedRay>
				<!--  An augmented ray for each sector leg with Local CRS -->
   				<augmentedRay>
					<featureReference><xsl:value-of select="$fid"/></featureReference>
				...
				<!--  An augmented Path for the sector arc Local CRS -->
   				<augmentedPath>
					...
					<!-- add a linestlye element -->
					<xsl:call-template name="simpleLineStyle">
						<xsl:with-param name="style"><xsl:value-of select="$lineStyle"/></xsl:with-param>
						<xsl:with-param name="width"><xsl:value-of select="$lineWidth"/></xsl:with-param>
		...
		<xsl:for-each select="sectorCharacteristic/lightSector[directionalCharacter/orientation/orientationValue and directionalCharacter/orientation/orientationValue != '']">
			...
			<xsl:variable name="SeawardOrient">
				<xsl:value-of select="directionalCharacter/orientation/orientationValue"/>
			</xsl:variable>
			...
			<!--  An augmented ray for each sector leg with Local CRS -->
   			<augmentedRay>
				...
				<displayPlane><xsl:value-of select="$displayPlane"/></displayPlane>
				<drawingPriority><xsl:value-of select="$drawingPriority"/></drawingPriority>
				<crs><xsl:text>GeographicCRS</xsl:text></crs>  
				<!-- add a linestlye element -->
				<xsl:call-template name="simpleLineStyle">
					<xsl:with-param name="style">dash</xsl:with-param>
					<xsl:with-param name="width">0.32</xsl:with-param>
			...
			<pointInstruction>
				...
				<xsl:element name="symbol">
					...
					<xsl:attribute name="rotation"><xsl:value-of select="$FromLightOrient"/></xsl:attribute> 
				</xsl:element>
			</pointInstruction>
			...
			<textInstruction>
				...
				<textPoint horizontalAlignment="Start" verticalAlignment="Top">
					<element>
						<text><xsl:value-of select="format-number($SeawardOrient, '000','dformat')"/><xsl:text> deg</xsl:text></text>
