...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="AvgWavePrd[@primitive='Point']" priority="1">
    <xsl:call-template name="select_AvgWavePrd">
  ...
  <xsl:template name="select_AvgWavePrd">
    <xsl:param name="viewingGroup"/>
    <!--  The viewing group  -->
    <xsl:param name="displayPlane"/>
    <!--  The display plane  -->
    <xsl:param name="drawingPriority"/>
    <xsl:variable name="AvgWavePrd">
      <xsl:value-of select="avgWavePrd"/>
    </xsl:variable>
    <xsl:variable name="Color">
      <xsl:choose>
        <xsl:when test="$AvgWavePrd = 8.0">
          <xsl:value-of select="'PLRTE'"/>
        </xsl:when>
        <xsl:when test="$AvgWavePrd > 7.1">
          <xsl:value-of select="'APLRT'"/>
        </xsl:when>
        <xsl:when test="$AvgWavePrd > 6.2">
          <xsl:value-of select="'CHYLW'"/>
        </xsl:when>
        <xsl:when test="$AvgWavePrd > 5.3">
          <xsl:value-of select="'RADLO'"/>
        </xsl:when>
        <xsl:when test="$AvgWavePrd > 4.4">
          <xsl:value-of select="'RESBL'"/>
        </xsl:when>
        <xsl:when test="$AvgWavePrd > 3.5">
          <xsl:value-of select="'DEPVS'"/>
        </xsl:when>
        <xsl:when test="$AvgWavePrd > 2.5">
  ...
  <!--<xsl:template match="S413_AvgWavePrd[@primitive='Point']" priority="1">
    <textInstruction>
      <featureReference>
        <xsl:value-of select="@id"/>
      </featureReference>
